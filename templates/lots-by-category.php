<?php
    ...
    foreach ($categories as $category) {
        $flipedCatList = array_flip($category);
        
        if (isset($flipedCatList[$code])) {
...
<div class="container">
    <section class="lots">
    <h2>Все лоты в категории <span>«<?= $chosenCategory; ?>»</span></h2>
    <ul class="lots__list">
        <?php if ($ads): ?>
        ...
        <ul class="pagination-list">
          <?php if ($currentPage > 1): ?>
            <li class="pagination-item pagination-item-prev">
              <a href="?source=lots-by-category&cat=<?= $_GET['cat'] ?>&page=<?= $currentPage - 1 ?>">Назад</a>
            </li>
          <?php endif; ?>

          <?php for ($i = 1; $i <= $totalPages; $i++): ?>
            <li class="pagination-item <?= $i === $currentPage ? 'pagination-item-active' : '' ?>">
              <a href="?source=lots-by-category&cat=<?= $_GET['cat'] ?>&page=<?= $i ?>"><?= $i ?></a>
            </li>
          <?php endfor; ?>

          <?php if ($currentPage < $totalPages): ?>
            <li class="pagination-item pagination-item-next">
              <a href="?source=lots-by-category&cat=<?= $_GET['cat'] ?>&cat=<?= $_GET['cat'] ?>&page=<?= $currentPage + 1 ?>">Вперед</a>
            </li>
          <?php endif; ?>
        </ul>
    <?php endif; ?>
